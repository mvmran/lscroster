...
interface ManagedInvitationEmailParams {
  churchName: string
  /** First name of the managing member receiving this email. */
  managerName: string
  /** Full name of the managed person they'll look after. */
  managedName: string
  inviteUrl: string
  expiresInDays: number
}
...
/**
 * Sent to a managing member when an admin asks them to look after a person who
 * has no email of their own (issue #89). It makes the on-behalf relationship
 * explicit and closes with a privacy reminder.
 */
export function managedInvitationEmail({
  churchName,
  managerName,
  managedName,
  inviteUrl,
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background-color:#ffffff;border-radius:12px;padding:32px;">
            ...
            <tr>
              <td style="font-size:15px;line-height:1.6;color:#3f3f46;">
                ...
                their requests will come to this email address.
              </td>
            </tr>
            <tr>
            ...
            <tr>
              <td style="padding-top:24px;font-size:12px;line-height:1.6;color:#a1a1aa;">
                Please respect ${managedName}'s privacy: only use this access on
                their behalf, and keep what you see to yourself and them. If you
