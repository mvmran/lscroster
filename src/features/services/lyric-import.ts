...
export function parseImportedLyrics(text: string): ParsedImport {
  const blocks: Block[] = splitAtHeaders(toLines(text)).map(
    ({ header, body }) => ({
      header,
  ...
  blocks.forEach((block, index) => {
    ...
    const sectionNative =
      block.native.length === 0 ? [] : [...blank, ...block.native];
    const sectionMeaning =
      block.meaning.length === 0 ? [] : [...blank, ...block.meaning];
  ...
  return {
    ...
    sections: blocks.filter((b) => b.header !== null).length,
...
export function appendImportedLyrics(
  ...
  const join = <T>(before: T[], after: T[]) => [...before, ...after];
  const gap = [""];
  ...
  const layer = (key: "native" | "meaning" | "chords") => {
    ...
    const lines = join(join(before, gap), after);
    return lines.some((line) => line.trim() !== "") ? lines.join("\n") : "";
  };

  return {
    lyrics: join(join(currentBase, gap), importedBase).join("\n"),
    native: layer("native"),
